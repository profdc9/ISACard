...
; Description	:	SD Device Command functions.
...
%define SDServer_AH_ALREADY_HAS_COMMAND_BYTE
%define SDServer_NO_ZERO_SECTOR_COUNTS

;--------------------------------------------------------------------
; SDCommand_OutputWithParameters
;	Parameters:
...
%endif
	...
	mov		dx, [di+DPT_SD.wPortIO8255]
; fall through to SDCommand_FallThroughToSDServer_SendReceive

ALIGN JUMP_ALIGN
SDCommand_FallThroughToSDServer_SendReceive:
; fall through to SDServer_SendReceive
%include "SDServer.asm"

%ifndef CHECK_FOR_UNUSED_ENTRYPOINTS
	%if SDCommand_FallThroughToSDServer_SendReceive <> SDServer_SendReceive
		%error "SDServer_SendReceive must be the first routine at the top of SDServer.asm in the Assembly_Library"
	%endif
%endif

ALIGN JUMP_ALIGN
SDCommand_ReturnError:
	stc
	ret

;--------------------------------------------------------------------
; SDCommand_IdentifyDeviceToBufferInESSIwithDriveSelectByteInBH
;	Parameters:
...
;     master scan.
;
	mov		dx, [cs:bp+IDEVARS.wBasePort]
	xor		ax, ax
...
.notfounddpt:
	...
	jz		SHORT SDCommand_ReturnError
...
.identifyDeviceInDX:
; fall through to SDCommand_FallThroughToSDServerScan_ScanForServer

ALIGN JUMP_ALIGN
SDCommand_FallThroughToSDServerScan_ScanForServer:
; fall through to SDServerScan_ScanForServer
%include "SDServerScan.asm"

%ifndef CHECK_FOR_UNUSED_ENTRYPOINTS
	%if SDCommand_FallThroughToSDServerScan_ScanForServer <> SDServerScan_ScanForServer
		%error "SDServerScan_ScanForServer must be the first routine at the top of SDServerScan.asm in the Assembly_Library"
	%endif
%endif
