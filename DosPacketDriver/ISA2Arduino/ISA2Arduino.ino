...
#define string_in_flash(x) F(x)
#else
#define PSTORE
#define strcpy_flash strcpy
#define string_in_flash(x) (x)
#endif
...
uint8_t eth_retrigger_interrupt_flag = 0;
Wiznet5500 eth(CS3);
#endif
...
extern "C" {
  void write_dbg_string(const char *c)
  {
#ifdef DEBUG_SERIAL
    SERIALPORT()->print(c);
#endif
  }
  void write_dbg_number(uint16_t n)
  {
#ifdef DEBUG_SERIAL
    SERIALPORT()->print(n, HEX);
#endif
  }
}
...
void do_initialize_ethernet(void)
{
  ...
  SERIALPORT()->println(string_in_flash("initialize ethernet"));
#endif
  for (uint8_t i = 0; i < 6; i++)
  {
    ...
    SERIALPORT()->print(string_in_flash(" "));
#endif
  }
  if (ethernet_initialized)
    eth.end();
  ethernet_initialized = 0;
  //cards_initialized = 0; last_drive = 255;
  if (eth.begin(mac_address, true))
  {
    //check_cards_initialized();
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
    SERIALPORT()->println(string_in_flash("initialized"));
#endif
  ...
  //check_cards_initialized();
#ifdef DEBUG_SERIAL
  SERIALPORT()->println(string_in_flash("not initialized"));
#endif
  write_dataport(1);
  return;
}
...
{
  uint16_t len;
  while ((len=eth.discardFrame()) != 0)
  {
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
    SERIALPORT()->print(string_in_flash("discard len "));
    SERIALPORT()->print(len, HEX);
#endif
...
void do_dummy_read_ethernet(void)
{
  ...
  temp = read_dataport();
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
    SERIALPORT()->println(string_in_flash("dummy read"));
#endif
  write_dataport(0);
  write_dataport(0);  
}

void retrigger_interrupt(void)
{
  if (ETHERNET_READ_OUTPUT_INTERRUPT_PIN())
  {
    ETHERNET_CLEAR_OUTPUT_INTERRUPT_PIN();
    delayMicroseconds(5);
  }
...
uint8_t poll_packet_available(void)
{
  static uint8_t n = 0;
  if ((++n) & 0x40)
  {
    n = 0;
    return eth.remainderFrame() != 0;
  }
  return 0;
}
...
void do_read_ethernet(void)
{
  uint16_t len;
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->print(string_in_flash("read eth" ));
#endif
  if (ethernet_initialized)
  {
    len = read_dataport();
...
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
    SERIALPORT()->print(string_in_flash("read len "));
    SERIALPORT()->print(len, HEX);
#endif
    len = eth.readFrame(NULL, len);
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
    SERIALPORT()->print(string_in_flash(" recv len "));
...
void do_send_ethernet(void)
{
  uint16_t len;
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->print (string_in_flash("send eth "));
#endif
  len = read_dataport();
  len |= ((uint16_t)read_dataport()) << 8;
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->print(string_in_flash("len "));
  SERIALPORT()->print(len, HEX);
#endif
  if (ethernet_initialized)
  {
    int16_t rtlen = eth.sendFrame(NULL, len);
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
    SERIALPORT()->print(string_in_flash(" len: "));
    SERIALPORT()->print(rtlen,HEX);
    SERIALPORT()->println(string_in_flash("\r\n"));
#endif
    if (rtlen != -1)
    {
      write_dataport(0);
  ...
  {
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->println(string_in_flash("not init!"));
#endif
  }
  while ((len--)>0)
    read_dataport();
...
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->println(string_in_flash("d="));
  SERIALPORT()->print(sizeof(fs));
  SERIALPORT()->print(string_in_flash(" f="));
  SERIALPORT()->print(freeRam());
  SERIALPORT()->print(string_in_flash(" s="));
...
void command_inquire(uint8_t drive)
{
  ...
  if (check_no_drive(drive))
  {
    ...
    {  
#ifdef DEBUG_SERIAL
      SERIALPORT()->println(string_in_flash("drive not initialized"));
#endif
      error_condition(0);
      return;
    }
...
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->print(string_in_flash("states "));
...
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
    SERIALPORT()->println(string_in_flash("transmitted response buffer"));
#endif
  write_dataport(0x0);

#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->print(string_in_flash("geometry "));
...
void loop()
{
  ...
  do
  {
    ...
    if (ethernet_initialized)
    {
      uint8_t current_loop = millis();
      uint8_t elapse = (++counter) >> 11;
      if ((!ethernet_waiting_for_read) || (eth_retrigger_interrupt_flag) || elapse)
      {
        if (elapse)
          counter = 0;
        if (eth_retrigger_interrupt_flag || (!ETHERNET_READ_INPUT_INTERRUPT_PIN()) || (poll_packet_available()))
        {
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->println(string_in_flash("set interrupt"));
#endif
          eth_retrigger_interrupt_flag = 0;
  ...
  if (ethernet_waiting_for_read)   // if we are interrupted, next command MUST be a read, otherwise ditch the patcket
  {
    ...
    if (instr == 0x84) 
    {
      do_read_ethernet();
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->println(string_in_flash("service read"));
#endif
      return;
    }
    eth_retrigger_interrupt_flag = 1;
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->print(string_in_flash("retrigger interrupt flag: "));
  SERIALPORT()->println(instr,HEX);
#endif
  }
#endif
  
  if (instr == 0xEE)              // special instant command code
  {
    write_dataport(0x47);
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->println(string_in_flash("g0xEE"));
#endif
    return;
  }
  if (instr == 0xED)              // special instant command code
  {
    write_dataport(0x9C);
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->println(string_in_flash("g0xED"));
#endif
    return;
  }
  ...
  {
#ifdef DEBUG_SERIAL
  SERIALPORT()->print(string_in_flash("badcmd "));
  SERIALPORT()->println(instr);
#endif
     return;
  }
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
  SERIALPORT()->print(string_in_flash("cmd "));
  SERIALPORT()->print(instr,HEX);
#endif
  cs.b[0] = instr;
...
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
    SERIALPORT()->println(string_in_flash("inquire"));
#endif
    return;
  }
  if (check_no_drive(drive))
...
#ifdef DEBUG_SERIAL
    SERIALPORT()->println(string_in_flash("inactive"));
#endif
    return;
  }
  ...
  if ((lba_sector + cs.cylinder_head_sector.count) > dg->sector_count)
  {
    error_condition(masked_command);
#ifdef DEBUG_SERIAL
    SERIALPORT()->print(string_in_flash("invalid lba "));
    SERIALPORT()->println(lba_sector);
#endif
    return;
  }
#if defined(DEBUG_SERIAL) && defined(DEBUG_STATUS)
    SERIALPORT()->print(string_in_flash("chs"));
  ...
  if ( ((drive != 0) && (slot1_state == SLOT_STATE_FILEDEV)) || ((drive == 0) && (slot0_state == SLOT_STATE_FILEDEV)) )
  {
      if ((!check_change_filesystem(drive)) || (f_lseek(&slotfile, lba_sector << 9) != FR_OK))
      {
          error_condition(masked_command);
#ifdef DEBUG_SERIAL
          SERIALPORT()->println(string_in_flash("file error"));
#endif
          return;
      }
      ...
      if (masked_command & 0x01)
      {
          while (count_sectors > 0)
          {
            ...
            if ((f_write(&slotfile, buf, 512, &br) != FR_OK) || (br != 512))
            {
                ...
                error_condition(masked_command);
#ifdef DEBUG_SERIAL
                SERIALPORT()->println(string_in_flash("file write error"));
#endif
                return; 
            }
          }
      ...
      } else
      {
          while (count_sectors > 0)
          {
            ...
            if ((f_read(&slotfile, buf, 512, &br) != FR_OK) || (br != 512))
            {
                ...
                error_condition(masked_command);
#ifdef DEBUG_SERIAL
                SERIALPORT()->println(string_in_flash("file read error"));
#endif
                return; 
            }
  ...
  if (masked_command & 0x01)
  {
      while (count_sectors > 0)
      {
         ...
         if (disk_write(drive, buf, lba_sector, 1) != 0)
         {
             ...
             error_condition(masked_command);
#ifdef DEBUG_SERIAL
                SERIALPORT()->println(string_in_flash("disk write error"));
#endif
             return;
         }
      }
   ...
   } else
   {
      while (count_sectors > 0)
      {
        ...
        if (disk_read(drive, buf, lba_sector, 1) != 0)
        {
             ...
             error_condition(masked_command);
#ifdef DEBUG_SERIAL
                SERIALPORT()->println(string_in_flash("disk read error"));
#endif
             return;  
        }
        transmit_512_bytes(buf);
